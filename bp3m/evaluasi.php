...
if ($_SESSION['_LevelID']==1) {
	$_arrWisuda = array(
	  "Setup Evaluasi~Setup",
	  "Kategori~Kategori",
	  "Daftar Pertanyaan~Pertanyaan",
	  "Hasil Survey~SurveyHasil"
	  );
  }
else {
$_arrWisuda = array(
...
// *** Main ***
TampilkanJudul("Evaluasi Dosen");
TampilkanTab($_arrWisuda, $_tabWisuda);
$gos = (empty($_REQUEST['gos']))? "Setup" : $_REQUEST['gos'];
$gos();
...
function TampilkanTahunWisuda() {
  ...
  echo "<table class=box cellspacing=1 align=center width=800>
    ...
    <tr>
        <td class=inp>Cari MK:</td>
        <td class=ul>
          <input type=text name='NamaMK' value='$_SESSION[NamaMK]' size=10 maxlength=50 />
          </td>
        <td class=inp>Cari Dosen:</td>
        <td class=ul>
          <input type=text name='NamaDosen' value='$_SESSION[NamaDosen]' size=10 maxlength=50 />
          <input type=submit name='SetParam' value='Refresh' />
          <input type=button name='Reset' value='Reset' 
            onClick=\"location='?mnux=$_SESSION[mnux]&_tabWisuda=$_SESSION[_tabWisuda]&gos=SurveyHasil&NamaMK=&NamaDosen='\" />
        </td>
        </tr>
    </form>
    </table>";
...
ESD;
  ...
  $lst->fields = <<<ESD
    ...
    if (m.Foto is NULL or m.FotoWisuda = '', 'img/tux001.jpg', concat('foto/wisudawan/kecil/small_',m.FotoWisuda)) as _Foto,
...
ESD;
  $lst->headerfmt = "<table class=box cellspacing=1 cellpadding=4 width=800>
    <tr><th class=ttl width=80>NPM</th>
        ...
        <th class=ttl>IPK | Toefl<hr size=1 color=silver />Predikat</th>
  ...
  $lst->detailfmt = "<tr>
    ...
    <td class=ul>
      ...
      <div align=right>
      <sub>
      <a href=#=MhswID= onClick=\"javascript:CetakIjazah('=MhswID=')\" title='Cetak Transkrip' data-rel='tooltip'>
      ...
      Ganti Foto</a>
      |</sub>
      <a href='#=MhswID=' onClick=\"javascript:TampilkanFoto('=MhswID=', '=Nama=', 'foto/wisudawan/=FotoWisuda=')\" title='=_Foto='> 
      <img src='=_Foto=' width=30 /></a>
      </div>
      </td>
    <td class=ul>
      <div align=center>&nbsp;=IPT= | =Toefl=</div>
  ...
  function CetakIjazah(MhswID) {
    _rnd = randomString();
    lnk = "$_SESSION[mnux].ijazah.php?_TrMhswID="+MhswID+"&_rnd="+_rnd;
    win2 = window.open(lnk, "", "width=700, height=600, scrollbars, status");
    if (win2.opener == null) childWindow.opener = self;
  }
  </script>
