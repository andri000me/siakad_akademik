...
// *** Parameters ***
$_kliringMhswID = $_SESSION['_Login'];
...
$mhsw = GetFields("mhsw m
    ...
    "m.KodeID='".KodeID."' and m.MhswID", $_kliringMhswID,
      "m.*, prd.Nama as _PRD, prg.Nama as _PRG,
      d.Nama as DSN, d.Gelar");

// *** Main ***
TampilkanJudul("Histori Nilai");
TampilkanHeader($mhsw);
RandomStringScript();
// validasi
if (!empty($_kliringMhswID)) {
...
$s = "SELECT m.MKKode,m.Nama,k.GradeNilai,m.SKS,m.MKSetara,m.MKID from mk m left outer join krs k on k.MKKode = m.MKKode and k.NA='N' AND  k.Final='Y' and k.Tinggi='*' and k.BobotNilai > 0 and k.MhswID = '$_SESSION[_Login]'
            where m.KurikulumID='$_SESSION[_kliringKurikulum]' order by Sesi,MKKode";
$r = _query($s);
  ?>
<table width="100%">
<tr>
...
<?php $n=0;
  while ($w = _fetch_array($r)){
    ...
    if(!empty($set)){
      ...
      $s1 = "SELECT m.MKKode,m.Nama,k.GradeNilai,m.SKS,k.BobotNilai,k.KRSID from mk m left outer join krs k on k.MKKode = m.MKKode and k.NA='N' AND  k.Final='Y' and k.Tinggi='*' and k.BobotNilai > 0 and k.MhswID = '$_SESSION[_Login]'
            where m.MKKode in ($_setara) and m.KurikulumID!='$_SESSION[_kliringKurikulum]' order by k.BobotNilai";
      //echo $s1;die();
      $r1 = _query($s1);$nn=0;
