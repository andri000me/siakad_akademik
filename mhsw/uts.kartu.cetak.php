...
// *** Main ***
$sudahbayar = "and (h.Biaya - h.Bayar + h.Tarik - h.Potongan) <= 0  AND h.Biaya>0";
$s = "select h.KHSID as KHSID, h.MhswID, m.Nama, h.IP, h.IPS,
...
function BuatIsinya($khs, $p) {
  ...
  $s = "select k.MKKode, k.SKS, j.NamaKelas, k.KRSID,
	  ...
	  j.MaxAbsen,j.JadwalID, left(jt.JamMulai,5) as JAM
    from krs k
  ...
  while ($w = _fetch_array($r)) {
	...
	$J = GetFields("jadwaluts js,utsmhsw u","u.JadwalUTSID = js.JadwalUTSID and u.MhswID='$khs[MhswID]' and js.JadwalID",$w[JadwalID],'js.RuangID as Ruang');
	...
	if($HitungMangkir <= $w['MaxAbsen'])
	{
		...
		$p->Cell(10, $t, $arrHari[$w['HRUTS']-1], $l, 0);
		$p->Cell(20, $t, $w['TGLUTS'].' '.$w['JAM'], $l, 0);
