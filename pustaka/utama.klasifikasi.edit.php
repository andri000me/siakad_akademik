...
HeaderSisfoKampus("Edit Kategori");
...
function Edit($md, $id, $bck) {
  if ($md == 0) {
    $jdl = "Edit Kategori";
    $w = GetFields('pustaka_klasifikasi', 'KlasifikasiID', $id, '*');
  ...
  <form action='../$_SESSION[mnux].klasifikasi.edit.php' method=POST>
  ...
  <tr><td class=inp>Nama Klasifikasi:</td>
...
function Simpan($md, $id, $bck) {
  $KlasifikasiID = $_REQUEST['id']+0;
  $Nama = sqling($_REQUEST['Nama']);
  $NA = sqling($_REQUEST['NA']);
  // Simpan
  if ($md == 0) {
    $s = "update pustaka_klasifikasi
      set Nama = '$Nama',
          NA  = '$NA'
  ...
  elseif ($md == 1) {
    $s = "insert into pustaka_klasifikasi
      (Nama,NA)
      values
      ('$Nama', '$NA')";
    $r = _query($s);
