...
function KonfirmasiTgl() {
  KonfirmasiTanggal("../$_SESSION[mnux].laporan.pinjam.telat.php", "Cetak");
}
...
function BuatIsinya($ProdiID, $p, $md) {
  ...
  echo '<table border=1>
    ...
    <td><strong style="font-size: 1.5em;">Keterlambatan<br />(hari)</td>
    </tr>'; $no=0;
    $date_criteria = 'AND (TO_DAYS(l.loan_date) BETWEEN TO_DAYS(\''.$_SESSION['TglMulai'].'\') AND
            ...
            b.title, l.loan_date, l.due_date,l.member_id,m.member_name, (TO_DAYS(DATE(NOW()))-TO_DAYS(due_date)) AS \'Overdue Days\'
            ...
            while ($gmd_d = _fetch_array($gmd_q)) {
              $no++;
                ...
                $output .= '<td class="'.$row_class.'"><strong style="font-size: 1.3em;">'.$gmd_d['Overdue Days'].'</strong></td>';
                $output .= '</tr>';
            }
    echo $output;
}
