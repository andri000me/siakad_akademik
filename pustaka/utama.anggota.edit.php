...
function Edit($md, $id, $bck) {
  ...
  elseif ($md == 1) {
    $jdl = "Tambah Anggota";
    $w = array();
$agt=GetaField('INFORMATION_SCHEMA.TABLES', "TABLE_SCHEMA = 'ubh_portal' AND   TABLE_NAME", 'pustaka_anggota', "AUTO_INCREMENT");
  ...
  <tr><td class=inp>Nama Institusi:<br /><sub>*) bila bukan mahasiswa internal</sub></td>
      ...
      </tr>".($md==0 ? "<tr><td class=inp>NA (Non-Aktif):</td>
...
function Simpan($md, $id, $bck) {
  ...
  elseif ($md == 1) {
    ...
    $s = "insert into pustaka_anggota
      (AnggotaID, Nama, Handphone, Email, Alamat, NamaInstitusi,InstitusiID, NA, LoginBuat, TanggalBuat)
      values
      ('$AnggotaID','$Nama', '$Handphone','$Email','$Alamat', '$NamaInstitusi','$_REQUEST[InstitusiID]','N', '$_SESSION[_Login]',now())";
    $r = _query($s);
