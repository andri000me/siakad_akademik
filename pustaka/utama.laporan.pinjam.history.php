...
function KonfirmasiTgl() {
  KonfirmasiTanggal("../$_SESSION[mnux].laporan.pinjam.history.php", "Cetak");
}

function Cetak(){
    header("Content-type:application/vnd.ms-excel");
    header("Content-Disposition:attachment;filename=History-Peminjaman-".$_SESSION['TglMulai']."-".$_SESSION['TglSelesai']);
    header("Expires:0");
...
function BuatIsinya($ProdiID, $p, $md) {
  ...
  echo '<table border=1>
    ...
    <td><strong style="font-size: 1.5em;">Status</td>
    </tr>'; $no=0;
    $date_criteria = 'AND (TO_DAYS(l.loan_date) BETWEEN TO_DAYS(\''.$_SESSION['TglMulai'].'\') AND
            ...
            b.title, l.loan_date, l.due_date,l.member_id,m.member_name, l.is_return as Status
            ...
            WHERE l.is_return in (0,1) ".( !empty($date_criteria)?$date_criteria:'' ));
            while ($gmd_d = _fetch_array($gmd_q)) {
              $no++;
                ...
                $output .= '<td class="'.$row_class.'"><strong style="font-size: 1.3em;">'.($gmd_d['Status']==1? "Kembali":"Pinjam").'</strong></td>';
                $output .= '</tr>';
            }
    echo $output;
}
