<?php	session_start();error_reporting(0);
	...
	*/
	...
	if (empty($MhswID)) die('Tidak bisa melanjutkan, data tidak tersedia');
	...
	$Skripsi 		= GetFields("tugasakhir", "MhswID", $MhswID, "*");
	
	// FIlter hanya untuk dosen PA yang bersangkutan saja
...
echo "
	...
	<div style='width:650px; text-align:center; background: #ffffff;'>";
			...
			echo "<div class='fakultas'>".($DataFakultas['FakultasID']=='08'? "PROGRAM ":"FAKULTAS ").strtoupper($DataFakultas['Nama'])."</div>
			...
			<tr>
			<td>".($DataFakultas['FakultasID']=='08'? "Program ":"Fakultas ")."</td><td>:</td><td>".str_replace(" Dan ", " dan ",ucwords(strtolower($DataFakultas['Nama'])))."</td>
			</tr>";
			...
			if($DataFakultas['Nama'] == "Hukum")
			{
				...
				echo "<tr>
				<td>Jurusan</td><td>:</td><td>".$DataProdi['Jurusan']."</td>
				</tr>
	...
	$whr			= (!empty($KurikulumID))? " AND m.KurikulumID='$KurikulumID' " : "AND m.KurikulumID='$KurikulumID'";
	//$lihat 			= ($_SESSION['_LevelID']==120? "and k.Final = 'Y'":"");
	$query3 		= _query("select m.Nama,kl.Nama as Klasifikasi, kl.Singkatan, m.MKID, m.MKKode, m.SKS, Max(k.NilaiAkhir), Max(k.BobotNilai) as BobotNilai, n.Nama as Grade
							 from krs k left outer join mk m on m.MKID=k.MKID 
										left outer join jenismk kl on kl.JenisMKID=m.JenisMKID 
										left outer join kurikulum u on u.KurikulumID=m.KurikulumID
										left outer join nilai n on n.Bobot = k.BobotNilai and n.ProdiID='$ProdiID'
										where
			...
			$tglyudisium1 = TanggalFormat($Skripsi['TglUjian']);
			...
			<tr><td valign='top'>Judul ".($DataFakultas['FakultasID']=='08' ? "Tesis":"Skripsi")."</td><td valign='top'>:</td><td><div style='border:1px solid #000000; height:65px; width:450px; padding:5px 10px 10px 5px'>".FixStr($Skripsi['Judul'])."</div></td></tr>
			</table></div><br />";
...
function BuatNilaiTertinggi($mhsw) {
  // Ambil semuanya dulu
  $s = "select k.KRSID, k.MKKode, k.MKID,k.BobotNilai, k.GradeNilai, k.SKS, k.Tinggi
