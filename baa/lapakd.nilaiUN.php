...
// *** Init PDF
$pdf = new PDF();
$pdf->SetTitle("Rekapitulasi Jumlah Mahasiswa Berdasarkan Nilai UN");
$pdf->AddPage();
$lbr = 190;
...
function BuatHeadernya($pendapatan, $p) {
  ...
  $p->Cell($lbr, $t, "Rekapitulasi Jumlah Mahasiswa Berdasarkan Nilai UN", 0, 1, 'C');
  $p->Ln($t);
  ...
  $p->Cell(13*sizeof($agama), $t, 'Nilai UN', 1, 0, 'C');
  $p->Ln($t);
...
function GetArrayPendapatan(&$arrJmlPendapatan) {
  ...
  $s = "select m.TahunID, count(m.MhswID) as JML, a.NilaiUjianTotal
    from mhsw m 
    left outer join pmb a on a.PMBID=m.PMBID
    where m.TahunID in (2014,2015)
    group by a.NilaiUjianTotal, m.TahunID";
  $r = _query($s);
  $arr = array();
  while ($w = _fetch_array($r)) {
    if (array_search($w['NilaiUjianTotal'], $arr) === false) $arr[] = $w['NilaiUjianTotal'];
    $arrJmlPendapatan[$w['TahunID']][$w['NilaiUjianTotal']] = $w['JML'];
