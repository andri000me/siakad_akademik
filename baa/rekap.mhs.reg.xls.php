  ...
  session_start();
	...
	$StatusAwalID = GetSetVar('StatusAwalID');
	 $_SESSION['_Bayar'] = ($_GET['byr'] == 'Y')? 'Y' : 'N';
    		$whr = array();
		//cek bayar
		if ($_SESSION['_Bayar'] == 'Y')   $whr[] = "((h.Bayar>0) or m.StatusAwalID='M' or m.StatusAwalID='S' or m.ProgramID='M' or m.ProgramID='P')";
		//cek sks
			//cek krs	
				...
				elseif ($_SESSION['_KRS']==3) 
				{ 
					$whr[] = "h.SetujuPA='Y'";
				}
				elseif ($_SESSION['_KRS']==4) 
				{ 
					$whr[] = "h.SetujuPA!='Y'";
				}
			if (!empty($_SESSION['StatusAwalID'])) 
				{ 
					$whr[]= "m.StatusAwalID='".$_SESSION['StatusAwalID']."'";
				}
...
header("Pragma: public");
 ...
 $s="select distinct(left(TahunID,5)) as TahunID from mhsw where TahunID>2001 and TahunID<=$_SESSION[ThnAkd] order by TahunID";
 $r=_query($s);
 $n=_num_rows($r);
 $n++;
 }
...
<?php  if (!empty($_SESSION['ThnAkd'])) { ?>
 ...
 <tr><td class=header align="center" colspan="<?php echo $_n; ?>"><strong> BIRO ADMINISTRASI AKADEMIK & KEMAHASISWAAN (BAAK)<BR />
	...
	<td align="center"><?php echo $w1[NMProdi].' '.$w1[Prog].'<br>'.$w1[ProdiID]; ?></td>
		<?php 
		$s2="SELECT count( h.MhswID ) as JML, h.ProdiID
			FROM khs h, mhsw m
...
";
		...
		$tot = 0;
		 	$s="select distinct(left(TahunID,5)) as TahunID from mhsw where TahunID>1999 and TahunID<=$_SESSION[ThnAkd] order by TahunID";
 			$r=_query($s);
		while ($w=_fetch_array($r)) {
			$s2="SELECT count( h.MhswID ) as JML, h.ProdiID
			FROM khs h, mhsw m
	...
	<?php 
		...
		$s="select distinct(left(TahunID,5)) as TahunID from mhsw where TahunID>2006 and TahunID<=$_SESSION[ThnAkd] order by TahunID";
 			$r=_query($s);
		while ($w=_fetch_array($r)) {
			$s2="SELECT count( h.MhswID ) as JML
			FROM khs h, mhsw m
