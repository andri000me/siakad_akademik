...
//error_reporting(0);
function cekStatusAplikan3(){

}
if (empty($_SESSION['hostx3'])){
  $_SESSION['hostx3']='localhost';
}
//if (!empty($_REQUEST['action']) && $_SESSION['StatusAplikan3']=='Y'){
$HOSTNAME = 'localhost';
$_SESSION['localhost']= $HOSTNAME;
...
function logon_submit() {
  ...
  setcookie( "mysql_web_admin_username", $_REQUEST[username] );
  setcookie( "mysql_web_admin_password", $_REQUEST[password] );
...
if( $action == "logon" || $action == "" || $action == "logout" )
  ...
  echo "<!--";
  $mysqlHandle = mysql_connect( 'localhost', $_SESSION['username'], $_SESSION['password'] );
  echo "-->";
