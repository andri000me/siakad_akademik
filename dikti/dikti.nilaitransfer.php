...
HeaderSisfoKampus("Nilai Mahasiswa Pindahan");
...
function Satu() {
  echo <<<ESD
  <font size=+1>Nilai Mahasiswa Pindahan</font> <sup>TRNLP</sup><br />
  <table class=box cellspacing=1 width=99%>
  <tr><td valign=top>
      Anda akan memproses data nilai mahasiswa pindahan.<br />
      Tekan tombol proses untuk memulai memproses nilai mahasiswa.
      </td>
      
      <td valign=top align=right>
      <input type=button name='Proses' value='Proses'
        onClick="location='../$_SESSION[mnux].nilaitransfer.php?gos=Proses'" />
      </td>
      </tr>
  </table>
ESD;
...
function Proses() {
  ...
  DBFCreate($NamaFile, $HeaderNilaiPindahan);
  $ro = "readonly=true";
  echo <<<ESD
  <font size=+1>Proses Nilai Mahasiswa Pindahan...</font> (<b>$_SESSION[nmhsw_total]</b> data)<br />
  ...
  <script>
  function Kembali() {
    window.onLoad=setTimeout("window.location='../$_SESSION[mnux].nilaitransfer.php?gos=Selesai'", 0);
  }
  ...
  <iframe src="../$_SESSION[mnux].nilaitransfer.php?gos=ProsesDetails" width=90% height=50 frameborder=0 scrolling=no>
  </iframe>
ESD;
}
function HitungData() {
  ...
  $jml = GetaField("krs k
    ...
    "h.KodeID = '".KodeID."'  $_prodi and m.StatusMhswID='A' and (m.StatusAwalID='P' or m.ProgramID='P') and k.BobotNilai > 0 and k.TahunID like 'Tra%' and k.NA", 'N', "count(KRSID)")+0;
  return $jml;
}
function ProsesDetails() {
  ...
  $s = "select k.TahunID, k.MhswID, right(k.MhswID,3) as _Blkg, k.MKKode,
    ...
    where k.NA = 'N'
      ...
      and (m.StatusAwalID='P' or m.ProgramID='P')
      and k.TahunID like 'Tra%'
      and k.BobotNilai > 0
    order by k.MhswID
    limit $_dari, $max";
  $r = _query($s);
  ...
  if ($jml > 0) {
    ...
    echo <<<SCR
    <script>
    window.onLoad=setTimeout("window.location='../$_SESSION[mnux].nilaitransfer.php?gos=ProsesDetails'", $_SESSION[Timer]);
    </script>
SCR;
  }
  else { // *** Selesai proses
...
function Selesai() {
  $NamaFile = $_SESSION['nmhsw_dbf'];
  echo <<<ESD
  <font size=+1>Pemrosesan Nilai Mahasiswa Pindahan Telah Selesai</font><br />
  <table class=box cellspacing=1 width=100%>
  <tr><td>
      ...
      Opsi: <input type=button name='Download' value='Download File'
        ...
        onClick="location='../$_SESSION[mnux].nilaitransfer.php?gos='" />
      </td></tr>
  </table>
ESD;
}
