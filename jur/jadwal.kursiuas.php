...
$pdf->SetTitle("Daftar Peserta Kuliah - $TahunID");
...
function BuatIsinya($jdwl, $p) {
  ...
  $s = "select k.MhswID, m.Nama, k._Presensi, k.KRSID, k.TahunID, k.MKKode
    from krs k
  ...
  $p->Cell(35, $t, 'N P M', 'B', 0);
  $p->Cell(35, $t, 'Teori', 'B', 0);
  $p->Cell(35, $t, 'Praktek', 'B', 0);
  ...
  while ($w = _fetch_array($r)) {
	$n++;
	$BayarTeori = GetaField("bipotmhsw", "TambahanNama like '$w[MKKode]%Teori:%' and TahunID='$w[TahunID]' And MhswID", $w['MhswID'],'Dibayar');
	$BayarPraktek = GetaField("bipotmhsw", "TambahanNama like '$w[MKKode]%Praktek%' and TahunID='$w[TahunID]' And MhswID", $w['MhswID'],'Dibayar');
    $p->SetFont('Helvetica', '', 9);
    $p->Cell(72, $t, $w['Nama'], 0, 0);
    $p->Cell(35, $t, $w['MhswID'], 0, 0);
    $p->Cell(35, $t, "Rp ".number_format($BayarTeori,0), 0, 0);
    $p->Cell(35, $t, "Rp ".number_format($BayarPraktek,0), 0, 0);
    $p->SetFont('Helvetica', 'B', 9);
    //$p->Cell(15, $t, $n, 0, 0, 'C');
    //$p->Cell(20, $t, '', 'B', 0, 'C');
    $p->Ln($t);
    $_BayarTeori += $BayarTeori;
    $_BayarPraktek += $BayarPraktek;
  }
  $p->SetFont('Helvetica', '', 9);
  $p->Cell(132, 2, ' ', 'B', 1);
  $p->Cell(107, $t, 'Jumlah Peserta : ' . $n, 0, 0);
  $p->Cell(35, $t, 'Rp ' .number_format($_BayarTeori,0), 0, 0);
  $p->Cell(35, $t, 'Rp ' .number_format($_BayarPraktek,0), 0, 1);
...
function BuatHeaderKursi($jdwl, $thn, $p) {
  $t = 6; $lbr = 190;
  $p->AddPage('P');
  HeaderLogo('PESERTA MATAKULIAH', $p, 'P');
