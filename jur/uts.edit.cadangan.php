<?php		session_start();
	...
	*/
	...
	$header .= " 
			...
			<div class='logo' style='text-align:center'>Daftar Hadir Ujian Tengah Semester</div>
			<br />
		...
		";
            ...
            echo "<html>
			<head>
			<title>Absensi Ujian Tengah Semester $dd1 $dd2</title>
			<style>
			body {
...
{ 	$start = $i*$maxentryperpage;
			echo $header;
	if ($jdwl['ProgramID']=='M' || $jdwl['ProgramID']=='P' || $jdwl['PerSKS']=='N'){
	$s1 = "select k.MhswID, upper(m.Nama) as Nama, m.TahunID as ThnMasuk, h.TahunID, p.JenjangID,m.ProgramID,h.Biaya,h.Potongan,h.Bayar,h.Tarik, k.MKKode
    from khs h, krs k
      left outer join mhsw m on m.MhswID = k.MhswID and m.KodeID = '".KodeID."'
	  left outer join prodi p on p.ProdiID = m.ProdiID
    where k.JadwalID = '$jdwl[JadwalID]'
	...
	$r1 = _query($s1);
			while($w = _fetch_array($r1))
				{
	if ($w['ProgramID']=='R' || $w['ProgramID']=='J') {
	  $cekBipot = GetFields("bipotmhsw b","b.MhswID='$w[MhswID]'
								AND b.TambahanNama like (concat('%$w[MKKode]%'))
								AND b.NA='N'
								AND b.TahunID", $w['TahunID'], "(b.Dibayar - (b.Jumlah*b.Besar)) as Bayar, (b.Jumlah*b.Besar) as Besaran, Dibayar");
  $tunggakan = ($w['Biaya'] - $w['Potongan']) - ($w['Bayar']) + $w['Tarik']+0;
    //if (($w['Biaya'] - $tunggakan)+0 > 0 ) {
		$PESAN = $w['MhswID'];
		if ($w['ThnMasuk']==date('Y')) {
					echo "<tr>
					<td style='text-align:right'>".$counter1.".</td>
					<td>".$w['MhswID']."</td>
					<td>".ucwords(strtolower($w['Nama']))."</td>
			...
			$PESAN .= "bukan mahasiswa baru";
			if ($cekBipot['Bayar'] >= 0 && $cekBipot['Dibayar'] >= $cekBipot['Besaran']) {
				echo "<tr>
					<td style='text-align:right'>".$counter1.".</td>
					<td>".$w['MhswID']."</td>
					<td>".ucwords(strtolower($w['Nama']))."</td>
				...
				$PESAN .= "-Belum Bayar";
				$cekStatusAwal = GetaField('mhsw',"MhswID", $w['MhswID'],"StatusAwalID");
				$cekMK = GetaField("krs k left outer join mk m on m.MKID=k.MKID", "k.MhswID='$w[MhswID]' and k.JadwalID='$jdwl[JadwalID]' and k.TahunID", $w['TahunID'], "m.PerSKS");
				if ($cekMK == 'N') {
					echo "<tr>
					<td style='text-align:right'>".$counter1.".</td>
					<td>".$w['MhswID']."</td>
					...
					if ($cekStatusAwal == 'S' || $cekStatusAwal == 'M') {
					echo "<tr>
					<td style='text-align:right'>".$counter1.".</td>
					<td>".$w['MhswID']."</td>
					<td>".ucwords(strtolower($w['Nama']))."</td>
	...
	elseif ($w['ProgramID']=='P' || $w['ProgramID']=='M') {
					echo "<tr>
					<td style='text-align:right'>".$counter1.".</td>
					<td>".$w['MhswID']."</td>
					<td>".ucwords(strtolower($w['Nama']))."</td>
...
{	
				echo $header;
	if ($jdwl['ProgramID']=='M' || $jdwl['PerSKS']=='N' || $jdwl['ProgramID']=='P'){
