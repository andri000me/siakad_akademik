...
HeaderSisfoKampus("Konfirmasi Menghapus Data Praktek Kerja", 1);

// *** Parameters ***
$PraktekKerjaID = GetSetVar('PraktekKerjaID');

// *** Main ***
TampilkanJudul("Konfirmasi Menghapus Data Praktek Kerja");
...
function fnSetupPenilaian($PraktekKerjaID) {
  ...
  <form name='frmLulus' action='../$_SESSION[mnux].hapus.php' method=POST>
  ...
  <tr><td class=ul colspan=4 align=center>
      <input type=submit name='btnSimpan' value='Hapus Sekarang (Tidak bisa dikembalikan)' />
...
function fnSimpan($PraktekKerjaID) {
  ...
  $GradeNilai = GetaField('nilai', "ProdiID='$prd' and Bobot='$BobotNilai' and KodeID",
    KodeID, "Nama");
  // Simpan
  $s = "UPDATE praktekkerja set NA='Y',
