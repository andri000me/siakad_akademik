...
// Buat Header
  ...
  $s1 = "select  ke.Nama as NamaKelas,do.Nama,do.Login,ju.DosenID as Pengawas,ja.Nama as MK,ja.MKKode,ju.RuangID as _UTSRuang,ju.JadwalUTSID,
				...
				from kelas ke, dosen do, jadwal ja, 
				jadwaluts ju left outer join hari huas on huas.HariID = date_format(ju.Tanggal, '%w')
				where ke.KelasID=ja.NamaKelas and do.Login=ja.DosenID and ja.JadwalID=ju.JadwalID and ju.TahunID='$_utsTahun' and ja.ProdiID='$_SESSION[_utsProdi]' order by ju.Tanggal, _UTSJamMulai, MK ,  NamaKelas DESC";
  ...
  while ($w = mysql_fetch_array($r1)) {
    $n++;
		$s2="select concat(Gelar1, ' ', Nama,', ',Gelar) as Nama from dosen where Login='$w[Login]'";
		$r2 = mysql_query($s2);
		while ($w2 = mysql_fetch_array($r2)) {
			$nmDosen=$w2['Nama'];
		}
			// seharusnya bukan NIDN, cuma karena sudah terlanjur jalan, semester 20141 harus diganti dengan Login
			$nmPengawas=$w['Pengawas']? GetaField('dosen',"NIDN",$w['Pengawas'],"concat(Gelar1, ' ', Nama,', ',Gelar)"):'';
