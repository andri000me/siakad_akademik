...
function BuatHeader($jdwl, $thn, $p) {
  $t = 5; $lbr = 190;
  $p->AddPage('P');
	$ok = "select k.*, UPPER(m.Nama) as NamaMhsw
    from krsmanual k
      left outer join mhsw m on k.MhswID = m.MhswID and m.KodeID = 'ITP'
    where k.MKKode = 'CES3252'
    order by m.MhswID";
  $uh = _query($ok);

  $arr = array();
  $arr[] = array('Mata Kuliah', ':', $uh['MKKode'] . '   ' . $uh['Nama'], 
    'Dosen Pengasuh', ':', $uh['DosenID']);
  $arr[] = array('Thn Akd', ':', 'Tahun 2011 Ganjil',
    'Program Studi', ':', $uh['Harga']);
  $arr[] = array('Semester / SKStes', ':',' ' ,
    'Hari / Tgl Ujian', ':', ' ');
  // Tampilkan
