...
// Buat Header
  ...
  $s1 = "select  ke.Nama as NamaKelas,do.Nama,do.Login,ju.DosenID as Pengawas,ja.Nama as MK,ja.MKKode,ju.RuangID as _UASRuang,ju.JadwalUASID,
				...
				where ke.KelasID=ja.NamaKelas and do.Login=ja.DosenID and ja.JadwalID=ju.JadwalID and ju.TahunID='$_uasTahun' and ja.ProdiID='$_SESSION[_uasProdi]' order by ju.Tanggal, _UASJamMulai, MK ,  NamaKelas DESC";
  ...
  while ($w = mysql_fetch_array($r1)) {
    $n++;
		...
		while ($w2 = mysql_fetch_array($r2)) {
			...
			$nmPengawas=$w['Pengawas']? GetaField('dosen',"Login",$w['Pengawas'],"concat(Gelar1, ' ', Nama,', ',Gelar)"):'';
echo "<tr><td>$n</td>
