...
function BuatIsinya($TahunID, $ProdiID, $p) {
  ...
  $s = "select j.JadwalID, j.MKKode, j.Nama, j.JumlahMhsw, d.Login, left(j.ProdiID,3) as Prodi, p.ProgramID,
        d.Nama as DSN
    from jadwal j 
  ...
  if($_dosenid != $w['Login'])
	...
	$bagidua = $w['JumlahMhsw'];
	if ($BelumIsi == $bagidua && $bagidua > 0){
	$n++;
    $ttlbelumisi += $BelumIsi;
	$ttljmlhmhsw += $w['JumlahMhsw'];
	$update = "UPDATE krs set NilaiAkhir='70', GradeNilai='B', BobotNilai='3', UAS='70' where JadwalID='".$w['JadwalID']."'";
	$_update = _query($update);
    $p->SetFont('Helvetica', '', 7);
    $p->Cell(10, $t, $n, 'LB', 0);
    $p->Cell(45, $t, $w['DSN'], 'B', 0);
    $p->Cell(25, $t, $w['MKKode'], 'B', 0);
	$p->Cell(10, $t, $w['Prodi'].'-'.$w['ProgramID'], 'B', 0);
