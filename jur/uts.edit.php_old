...
$gos($md, $id, $jutsid, $jutsid2);

// *** Functions ***
function Edit($md, $id, $jutsid, $jutsid2) {
  $w = GetFields('kelas kl, jadwal j', 'kl.KelasID=j.NamaKelas and j.JadwalID', $id, 'kl.Nama as NamaKLS, j.*');
...
END;
  ...
  <tr><td class=inp>Dosen Pengawas:</td>
      ...
      <input type=text name='UTSDosen2' value='$w0[UTSDosen]' size=30 maxlength=50 onKeyUp="javascript:CariDosen2('$_SESSION[_utsProdi]', 'frmJadwalUTS')" />
      <div style='text-align:right'>
      &raquo;
      <a href='#'
        onClick="javascript:CariDosen2('$_SESSION[_utsProdi]', 'frmJadwalUTS')" />Cari...</a> |
...
function Simpan($md, $id, $jutsid) {
  ...
  if (empty($oke)) {
	...
	if ($w[UTSRuangID] !='') {
	  ...
	  DaftarkanMhswKeRuangUTS($w, $JadwalUTSID1);
	  }
	 if ($w[UTSRuangID2] !='') {
	  $s = "insert into jadwaluts
        (KodeID, TahunID, JadwalID, DosenID, 
...
{	//echo "JADWALUTSID=$JadwalUTSID1 ,JADWALID=$w[JadwalID]<br>";
    ...
    {   $limit2= $w['UTSKapasitas2']+0;
		...
		$s4 = "select k.MhswID from krs k,khs h where h.MhswID=k.MhswID and h.TahunID=k.TahunID and (h.Bayar>0 or h.Potongan>0) and k.JadwalID='$w[JadwalID]' and k.KodeID='".KodeID."' order by k.MhswID limit $batas, $limit2";
		$r4 = _query($s4);
		$n4 = _num_rows($r4);
		echo "JUMLAH: $n2<br>";
...
function DaftarkanMhswKeRuangUTS($w, $JadwalUTSID1)
{	//echo "JADWALUTSID=$JadwalUTSID1 ,JADWALID=$w[JadwalID]<br>";
	if($JadwalUTSID1+0 != 0)
    {   
		$limit1= $w['UTSKapasitas']+0;
		$s6 = "select k.MhswID from krs k,khs h where h.MhswID=k.MhswID and h.TahunID=k.TahunID and (h.Bayar>0 or h.Potongan>0) and k.JadwalID='$w[JadwalID]' and k.KodeID='".KodeID."' order by k.MhswID limit 0,$limit1";
		$r2 = _query($s6);
		$n2 = _num_rows($r2);
		echo "JUMLAH: $n2<br>";
...
function JdwlUTSScript() {
  ...
  function CariRuang(ProdiID, frm, kapasitasR) {
    if (eval(frm + ".UTSRuangID.value != ''")) {
      ...
      showRuang(ProdiID, frm, eval(frm +".UTSRuangID.value"), 'cariruang', kapasitasR, eval("frmJadwalUTS.UTSTanggal_y.value"),eval("frmJadwalUTS.UTSTanggal_m.value"),eval("frmJadwalUTS.UTSTanggal_d.value"),eval("frmJadwalUTS.UTSJamMulai_h.value"),eval("frmJadwalUTS.UTSJamMulai_n.value"), eval("frmJadwalUTS.UTSJamSelesai_h.value"),eval("frmJadwalUTS.UTSJamSelesai_n.value"), eval(frm +".TahunID.value"));
  ...
  function CariDosen(ProdiID, frm) {
    if (eval(frm + ".UTSDosen.value != ''")) {
      ...
      showDosen(ProdiID, frm, eval(frm +".UTSDosen.value"), 'caridosen', eval(frm +".UTSTanggal_y.value"),eval(frm 		+".UTSTanggal_m.value"),eval(frm +".UTSTanggal_d.value"),eval(frm +".UTSJamMulai_h.value"),eval(frm +".UTSJamMulai_n.value"), eval(frm +".UTSJamSelesai_h.value"),eval(frm +".UTSJamSelesai_n.value"), eval(frm +".TahunID.value"));
