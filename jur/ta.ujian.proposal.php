...
HeaderSisfoKampus("Setup Ujian Seminar Proposal", 1);
CekBolehAksesModul();
echo <<<SCR
...
// *** Main ***
TampilkanJudul("Setup Ujian Seminar Proposal");
...
function TampilkanSetupUjianTA() {
  $ta = GetFields("ta t
    ...
    date_format(t.TglMulai, '%d-%m-%Y') as _TglMulai,JamUjianProposal,
...
function TampilkanHeaderTA($ta) {
  ...
  $_TglUjian = ($ta['TglUjianProposal'] == '0000-00-00')? date('Y-m-d') : $ta['TglUjianProposal'];
  $TglUjian = GetDateOption($_TglUjian, 'TglUjian');
  $JamUjian = GetTimeOption($ta['JamUjianProposal'], 'JamUjian');
  $NamaPenguji = GetaField('dosen', "KodeID='".KodeID."' and Login", $ta['PengujiProposal'], 'Nama');
  echo <<<SCR
  <table class=bsc cellspacing=1 width=100%>
  <form name='frmTA' action='../$_SESSION[mnux].ujian.proposal.php' method=POST />
  ...
  <tr><td class=inp>Tgl Ujian Seminar Proposal:</td>
      <td class=ul colspan=3>
      $TglUjian Jam $JamUjian
      </td></tr>
  <tr><td class=inp>Pemimpin Penguji:</td>
      <td class=ul colspan=3>
      <input type=text name='DosenID' value='$ta[PengujiProposal]' size=10 maxlength=50 />
  ...
  <tr><td class=ul colspan=4 align=center>
      ...
      <input type=button name='Refresh' value='Refresh'
        onClick="location='../$_SESSION[mnux].ujian.proposal.php?TAID=$ta[TAID]'" />
...
function TampilkanDosenPenguji($ta) {
  ...
  function HapusDosenPenguji(id) {
    if (confirm("Anda yakin akan menghapus dosen penguji ini?")) {
      window.location = "../$_SESSION[mnux].ujian.proposal.php?gos=HapusDosenPenguji&TAID=$ta[TAID]&id="+id;
    }
  }
  </script>
SCR;
...
ESD;
  BerhasilSimpan("../$_SESSION[mnux].ujian.proposal.php?TAID=$TAID", 1);
}
function DivTambahDosenPenguji($ta) {
  ...
  echo <<<SCR
  <table class=bsc cellspacing=1 width=100%>
  <form name='frmDosen' action='../$_SESSION[mnux].ujian.proposal.php' method=POST
...
function TambahDosenPenguji() {
  ...
  $cek = GetFields('ta', "TAID='$TAID' and PengujiProposal",$DosenID,'*');
  $cek1 = GetFields('tadosen', "TAID='$TAID' and Tipe=2 and DosenID",$DosenID,'*');
  $ada = GetaField('tadosen', "TAID='$TAID' and Tipe=2 and DosenID", $DosenID, 'TADosenID')+0;
  if(!empty($cek)){
    die(ErrorMsg('Error', 
      ...
      <hr size=1 color=silver />
       Opsi: <input type=button name='Kembali' value='Kembali'
        onClick=\"location='../$_SESSION[mnux].ujian.proposal.php?TAID=$TAID'\" />
...
function Simpan() {
  ...
  $s = "update ta
    set PengujiProposal = '$DosenID', TglUjianProposal = '$TglUjian', JamUjianProposal='$JamUjian',
