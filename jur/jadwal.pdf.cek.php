...
function BuatFooter($thn, $p) {
  ...
  $p->Ln(4);
  if ($_SESSION['_jdwlProg']=='R') {
  $p->Cell(8, $t, '*/ Jadwal yang diatur diluar jam perkuliahan bersifat sementara. Untuk informasi, silakan konfirmasi ke Prodi masing-masing.', 0, 1);
  }
  $p->Cell($mrg);
...
function AmbilJadwal($thn, $p) {
  ...
  $p->Cell(9, $t, 'SKS', 1, 0);
  $p->Cell(60, $t, 'Dosen Pengasuh', 1, 0);
  $p->Cell(30, $t, 'Praktek', 1, 0, 'C');
  $p->Cell(30, $t, 'Dihitung', 1, 0, 'C');
  ...
  $s = "select j.*,
	  ...
	  k.Nama AS namaKelas, mk.SKSPraktikum, mk.PerSKS
    from jadwal j
  ...
  while ($w = _fetch_array($r)) {
    ...
    */
    ...
    $p->Cell(60, $t, $w['DSN'], 1);
    $p->Cell(30, $t, $w['SKSPraktikum'], 1, 0, 'C');
    $p->Cell(30, $t, ($w['PerSKS']=='Y'? 'Ya':'Tidak'), 1, 0, 'C');
    $p->Cell(10, $t, $jMhswKRS.'/'. $jMhsw, 1, 0, 'R');
    $p->Ln($t);
  }
}
function BuatHeader($thn, $p) {
  ...
  $p->Cell(90, 6, "Prg Studi: " . $prodi . " " . $jenjang['Nama'], 0, 0);
