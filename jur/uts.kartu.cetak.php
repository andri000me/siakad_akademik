...
$s = "select h.KHSID as KHSID, h.MhswID, m.Nama, h.IP, h.IPS,
    ...
    where h.TahunID = '$_SESSION[TahunID]'
      ...
      $sudahbayar
	  group by h.KHSID
    order by h.MhswID $_SESSION[limit]";
$r = _query($s);
...
function BuatIsinya($khs, $p) {
  ...
  while ($w = _fetch_array($r)) {
	...
	$J = GetFields("jadwaluts js,utsmhsw u","u.JadwalUTSID=js.JadwalUTSID and u.MhswID='$khs[MhswID]' and js.JadwalID",$w['JadwalID']
					,'left(js.JamMulai,5) as JAM,js.RuangID as Ruang');
    ...
    $p->SetFont('Helvetica', '', 6);
		...
		$p->Cell(20, $t, $w['TGLUTS'].' '.$J['JAM'], $l, 0);
