...
function fnEdit() {
  ...
  if ($md == 0) {
    $w = GetFields('aplikan', "AplikanID='$id' and KodeID", KodeID, '*,format(NilaiSekolah/JumlahPelajaran,2) as NilaiRata');
...
ESD;
  ...
  $r8=_query($s8);
  $defPropinsi= GetaField('propinsi',"PropinsiID",$w[Propinsi],"NamaPropinsi");
  $defKabupaten= GetaField('propinsi',"DaerahID='$w[Kabupaten]' AND PropinsiID",$w[Propinsi],"NamaDaerah");
  $optionProp = "<option value='$w[Propinsi]' selected>$defPropinsi</option>";
  while ($w8 = _fetch_array($r8)) {
  if ($w8['PropinsiID']==$propinsiID) {
  $optionProp .= "<option value='$propinsiID' selected>$w8[NamaPropinsi]</option>";
  }
  else{
  ...
  //Default Sekolah
 $defSek = GetFields('asalsekolah',"SekolahID", $asalsekolah,'SekolahID,Nama,NamaPropinsi,NamaKabupaten');
 if (!empty($defSek)) {
  $defSekolah =  "<option value='$defSek[SekolahID]'>$defSek[Nama]</option>";
    $SumberSekolah = "<option value='sma' selected>SMA/SMK/MA/Sederajat</option>
	  <option value='pt'>Perguruan Tinggi</option>";
      $propinsiSekolahAPL = "<B>Propinsi $defSek[NamaPropinsi], $defSek[NamaKabupaten]</B>";
  }
  else {
  $defPT = GetFields('perguruantinggi',"PerguruanTinggiID", $asalsekolah,'PerguruanTinggiID,Nama,Kota');
 if (!empty($defPT)) {
  $defSekolah =  "<option value='$defPT[PerguruanTinggiID]'>$defPT[Nama]</option>";
  $propinsiSekolahAPL = "<b>$defPT[Kota]</b>";
    $SumberSekolah = "<option value='sma'>SMA/SMK/MA/Sederajat</option><option value='pt' selected>Perguruan Tinggi</option>";
  }
  }
  
  //Sumber sekolah
  if ($md==1) {
  $SumberSekolah = "<option value='sma'>SMA/SMK/MA/Sederajat</option>
	  <option value='pt'>Perguruan Tinggi</option>";
   }
   
  //Nilai Rata-rata
  $rata = " Nilai rata-rata: $w[NilaiRata]";
  
  
  // Tampilkan
  CheckFormScript("PresenterID");
  echo <<<ESD
  ...
  <tr><td class=inp>Nomer Aplikan :</td>
	  <td class=ul1>$_id, Password:$w[HashPassword]</td>
      </tr>
  ...
  <tr><td class=inp>Propinsi/Kabupaten/Kota : </td>
	  ...
	  <option value='$w[Kabupaten]'>$defKabupaten</option>
</select>
      </td>
      </tr>
 ...
 <tr><td class=inp>Cari Sekolah : </td>
      <td class=ul1 id="PilihPropID">$propinsiSekolahAPL
      </td>
      </tr>
  ...
  <tr><td class=inp>Rata-rata UN :</td>
      <td class=ul1>
      <input type=text name='NilaiSekolah' value='$w[NilaiSekolah]' size=5 maxlength=5 />
      </td>
      </tr>
	    <tr><td class=inp>Jumlah Matapelajaran :</td>
      <td class=ul1>
      <input type=text name='MataPelajaran' value='$w[JumlahPelajaran]' size=5 maxlength=2  /> $rata
      </td>
      </tr>
...
function fnSave() {
  ...
  $Kabupaten = sqling($_REQUEST['kabupaten']);
  ...
  $AsalSekolah = sqling($_REQUEST['NamaSekolah']);
  ...
  if ($md == 0) {
    $s = "update aplikan
      set AplikanID = '$AplikanID', 
          ...
          Telepon = '$Telepon', Handphone = '$Handphone', Email = '$Email', Kabupaten = '$Kabupaten',
          AsalSekolah = '$AsalSekolah', AlamatSekolah='$AlamatSekolah', JurusanSekolah = '$JurusanSekolah', TahunLulus = '$TahunLulus', 
		  SudahBekerja = '$SudahBekerja', NilaiSekolah = '$NilaiSekolah', JumlahPelajaran = '$MataPelajaran',
          NamaAyah = '$NamaAyah', PendidikanAyah = '$PendidikanAyah', PekerjaanAyah = '$PekerjaanAyah',
          ProdiID='$ProdiID', ProgramID = '$Program', 
          Catatan = '$Catatan', SumberInformasi = '$SumberInfo', CatatanPresenter = '$CatatanPresenter',
  ...
  elseif ($md == 1) {
	...
	$s = "insert into aplikan
      ...
      '$Telepon', '$Handphone', '$Email',
      '$AsalSekolah', '$AlamatSekolah', '$JurusanSekolah', '$TahunLulus', 
	  '$SudahBekerja', '$NilaiSekolah', '$MataPelajaran',
      '$NamaAyah', '$PendidikanAyah', '$PekerjaanAyah',
      '$ProdiID', '$Program',
      '$Catatan', '$SumberInfo',
	  '$AplikanID', md5('$HashPassword'), '$HashPassword',
