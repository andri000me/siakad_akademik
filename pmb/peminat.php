...
$_pmbol = $_POST['_pmbol'];
$_SESSION['serverSPMB'] = "http://sisfo.itp.ac.id/spmb";
...
function TampilkanHeaderAplikan($gelombang) {
  ...
  <tr>
      <td class=inp width=80>Cari:</td>
      <td class=ul1>
        <input type=text name='_apliNama' value='$_SESSION[_apliNama]' size=20 maxlength=50 placeholder='Nama/AplikanID' />
...
<td class=ul1 colspan=2>
	  ...
	  <tr><td class=ul1 colspan=2 valign=middle>&nbsp;</td>
  ...
  <tr><td class=ul1> <input type=button name='btnTambahAplikan' value='Tambah Aplikan'
      ...
      <td class=ul1 align=right>Cetak Daftar Aplikan <a href='pmb/peminat.xls.php'><img src='img/printer2.gif' title='Cetak Aplikan Gelombang ini' ></a>
      </td>
  ...
  function fnEditHistory(gel, md, id) {
   ...
   function fnLihatSPMB(PMBPeriodID) {
        lnk = "$_SESSION[serverSPMB]/pendaftar.php?periode="+PMBPeriodID+"&gos=";
        win2 = window.open(lnk, "", "width='100%', height=600, top=0,scrollbars");
        if (win2.opener == null) childWindow.opener = self;
      }
  </script>
...
var xmlhttp = createRequestObject();
	function rubah(pilih)
	{
		...
		if (!AplID) return;
		xmlhttp.open('get', 'http://url.ac.id/spmb/export.php?aplid='+AplID, true);
...
function DftrAplikan($gelombang) {
  ...
  // Filter formulir
  $whr = array();
  if (!empty($_SESSION['_apliNama'])) $whr[] = "(a.Nama like '%$_SESSION[_apliNama]%' or a.AplikanID like '%$_SESSION[_apliNama]%')";
  ...
  $lst->tables = "aplikan a
	...
	where a.KodeID = '".KodeID."' AND a.NA='N'
    $_whr
  order by a.PMBPeriodID desc, a.AplikanID";
