...
// *** Parameters ***
$id = sqling($_REQUEST['bayarid']);
//$id = '200820009';

$kwi = GetFields('pmbklinikbayar', "KodeID='".KodeID."' and PMBKlinikBayarID", $id, "*");
if (empty($kwi))
  die(ErrorMsg('Error',
...
$pdf = new PDF('P', 'mm', 'A4');
$pdf->SetTitle("Kwitansi Pembayaran Uang Tes Kesehatan");
$pdf->AddPage();
...
function BuatFooter($kwi, $gels, $p) {
  ...
  $r = _query($s);
  $w = _fetch_array($r);
  $p->Cell($knn, $t, GetaField('karyawan',"Login",$kwi['LoginBuat'],"Nama"), 0, 1, 'C');
...
function BuatIsinya($kwi, $gels, $p) {
  ...
  $p->Cell($mrg);
  $p->Cell(30, $t, '', 0, 0);
  $p->Cell(103, $t, $kwi['PMBKlinikBayarID'], 0, 1, 'R');
  $p->Ln(6);
  ...
  $p->Cell($mrg);
  $p->Cell($k2, $t, '', 0, 0);
  $p->Cell($k, $t, 'Uang Tes Kesehatan  ' . $gels['Nama'], 0, 1);
  
  // Baris 5 - Program Studi
  $p->Cell($mrg);
  $p->Cell($k2, $t, '', 0, 0);
	$Aplikan = GetFields('pmb','AplikanID',$kwi['AplikanID'],'*');
  ...
  $current_date = TanggalFormat(date('Y-m-d'));
  $p->Cell($mrg);
  $p->Cell($k2+10, $t, '', 0, 0);
  $p->Cell($k-20, $t, $kwi['Jumlah'], 0, 0);
